...
test_data = [
    ...
    pytest.param(
        DUnion(FloatString, IntString), FloatString, id="str_types_merge"
    ),
    pytest.param(
    ...
    pytest.param(DUnion(FloatString, str), str, id="union_str_str_based"),
    pytest.param(
        DUnion(DList(int), DList(str), str),
        DUnion(DList(DUnion(int, str)), str),
        id="union_of_lists",
    ),
    pytest.param(DUnion(Unknown, str), str, id="unknown_vs_single"),
    pytest.param(
        DUnion(Unknown, str, int), DUnion(str, int), id="unknown_vs_multi"
    ),
    pytest.param(
    ...
    pytest.param(DUnion(Null, str), DOptional(str), id="optional_str"),
    pytest.param(
        DUnion(Null, DList(str)), DOptional(DList(str)), id="optional_list"
    ),
    pytest.param(
    ...
    pytest.param(DUnion(Null, str, Null), DOptional(str), id="optional_str"),
    pytest.param(
        DUnion(DDict(str), DDict(str), DDict(str)), DDict(str), id="dict_union"
    ),
    pytest.param(
