...
test_path_split_data = [
    ...
    pytest.param(
        "/tmp/test/file.txt", ["/", "tmp", "test", "file.txt"], id="unix path"
    ),
    pytest.param("test/*.txt", ["test", "*.txt"], id="pattern"),
    pytest.param(
        "test/**/*.txt", ["test", "**", "*.txt"], id="recursive pattern"
    ),
]

if sys.platform.startswith("win"):
    test_path_split_data.append(
        pytest.param(
            "X:/test/file.txt", ["X:/", "test", "file.txt"], id="windows path"
        )
    )
else:
    test_path_split_data.append(
...
test_iter_json_file_data = [
    ...
    pytest.param(
        (json.load((path / "data" / "photos.json").open()), "items"),
...
def test_process_path(value, expected):
    result = {str(p).replace("\\", "/") for p in process_path(value)}
    assert result == expected, f"(in value: {value})"
