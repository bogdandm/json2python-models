...
test_data = [
    pytest.param(
        (
            "test_response",
            {
                "point_south": {"x": 0.4, "y": 0.2},
                "point_north": {"x": 0.4, "y": 0.2},
                "point_west": {"x": 0.4, "y": 0.2},
                "point_east": {"x": 0.4, "y": 0.2},
            },
        ),
        {
            ("TestResponse", False),
            ("PointEast_PointNorth_PointSouth_PointWest", True),
        },
        id="merge_models",
    ),
    pytest.param(
    ...
    pytest.param(
        (b"Response", {"point_south": {"x": 0.4, "y": 0.2}}),
    ...
    pytest.param(
        (
            "Point",
            {"x": 0.4, "y": 0.2, "sub": [{"x": 0.3, "y": 0.7, "sub": []}]},
        ),
        {("Point", False)},
        id="merge_with_name",
    ),
    pytest.param(
        ("Response", {"cats": {"a": "asdasd"}, "cat": {"b": "dfgertdfgdfg"}}),
...
def test_models_names(
    ...
    if isinstance(model_name, bytes):
        models_registry.process_meta_data(
            fields, model_name=model_name.decode()
        )
    ...
    names = {
        (model.name, model.is_name_generated)
