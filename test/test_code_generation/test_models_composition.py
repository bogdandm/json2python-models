...
test_extract_root_data = [
    pytest.param(
        ...
        {"Item": {"TestModelA", "TestModelB"}, "TestModelA": set()},
    ),
    pytest.param(
        [
            ("TestModelA", {"count": 1, "items": [{"x": 0.5, "y": 0.1}]}),
        ...
        {"Item": {"TestModelA_TestModelB"}, "TestModelA_TestModelB": set()},
        id="merge_root",
    ),
]
...
def test_extract_root(
    ...
    names_map.update(
        {model.name: model.index for model in models_registry.models}
    )
...
def _test_compose_models(
    ...
    def check(
        nested_value: List[dict], nested_expected: List[Tuple[str, list]]
    ):
        for model_dict, (model_name, nested) in zip(
            nested_value, nested_expected
        ):
            assert model_dict["model"].name == model_name, str(nested_value)
            assert len(model_dict["nested"]) == len(
                nested
            ), f"(Parent model is {model_name})"
...
test_compose_models_data = [
    pytest.param(
        [("A", {"field1": int, "item": {"another_field": float}})],
        [("A", [("Item", [])])],
        {},
        id="basic_test",
    ),
    pytest.param(
        [
            ("RootA", {"item": {"field": float}}),
            (
                "RootB",
                {"item": {"field": float}, "idontwantrootmodelstomerge": bool},
            ),
        ],
        [("Item", []), ("RootA", []), ("RootB", [])],
        {},
    ...
    pytest.param(
        [
            (
                "Root",
                {
                    "model_a": {"field_a": {"field": float}},
                    "model_b": {"field_b": {"field": float}},
                },
            ),
        ],
        [
    ...
    pytest.param(
        [
            ("RootItem", {"field": float}),
            ("RootA", {"item": {"field": float}, **base_dict}),
            ("RootB", base_dict),
        ],
        [("RootItem", []), ("RootA_RootB", [])],
        {},
    ...
    pytest.param(
        [
            (
                "Root",
                {
                    "model_a": {"field_a": {"field": {"nested_field": float}}},
                    "model_b": {"field_b": {"field": {"nested_field": float}}},
                },
            ),
        ],
        [
...
@pytest.mark.parametrize(
    "value,expected,expected_mapping", test_compose_models_data
)
...
@pytest.mark.parametrize(
    "value,expected,expected_mapping", test_compose_models_flat_data
)
