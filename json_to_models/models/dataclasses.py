...
DEFAULT_ORDER = (("default", "default_factory"), "*", ("metadata",))


class DataclassModelCodeGenerator(GenericModelCodeGenerator):
    DC_DECORATOR = template(
        f"dataclass{{% if kwargs %}}({KWAGRS_TEMPLATE}){{% endif %}}"
    )
    ...
    def decorators(self) -> Tuple[ImportPathList, List[str]]:
        ...
        decorators.insert(
            0, self.DC_DECORATOR.render(kwargs=self.dataclass_kwargs)
        )
    ...
    def field_data(
        ...
        if optional:
            ...
            if isinstance(meta.type, DList):
                ...
                if isclass(meta.type) and issubclass(
                    meta.type, StringSerializable
                ):
                    pass
        ...
        if (
            len(body_kwargs) == 1
            and next(iter(body_kwargs.keys())) == "default"
        ):
            data["body"] = body_kwargs["default"]
        elif body_kwargs:
            data["body"] = self.DC_FIELD.render(
